...
const server = http.createServer(async (req, res) => {
    ...
    setHeaders(res, origin);
                                   ...
                                   // POST /notes — body: { caseName, note }
                                   if (req.method === 'POST' && url === '/notes') {
                                         try {
                                                 const body = await readBody(req);
                                                 const { caseName, note } = body;
                                                 if (!caseName || !note) {
                                                           res.writeHead(400);
                                                           res.end(JSON.stringify({ error: 'caseName and note required' }));
                                                           return;
                                                 }
                                                 await pool.query(`
                                                         INSERT INTO case_notes (case_name, note, updated_at)
                                                                 VALUES ($1, $2, NOW())
                                                                         ON CONFLICT (case_name) DO UPDATE SET note = $2, updated_at = NOW()
                                                                               `, [caseName, note]);
                                                 res.writeHead(200);
                                                 res.end(JSON.stringify({ ok: true }));
                                         } catch (e) {
